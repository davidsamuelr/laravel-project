...
@endif
<br />
@unless($providers[0]['status'] == 'S') {{-- se retorno da condição for false --}}
    Fornecedor inativo
@endunless

<br /><br />

@isset($providers) {{-- retorna true se variável estiver definida --}}
    Variável definida<br />
    Fornecedor: {{ $providers[1]['nome'] }}<br />
    Status: {{ $providers[1]['status'] }}<br />
    @isset($providers[0]['cnpj']) {{-- retorna true se índice do array estiver definido --}}
        
        CNPJ: {{$providers[0]['cnpj']}}

        @empty($providers[0]['cnpj']) {{-- retorna true se variável ou índice do array estiver vazio --}}
            - Índice vazio
        @endempty
        <br />
